...
// VARIABLES CERO
volatile uint8_t inicio_cero = 0;
volatile uint8_t init_cont_cero = 0;
volatile uint16_t cont_cero = 0;
volatile uint8_t init_cont_barrera = 0;
volatile uint16_t cont_barrera = 0;
...
void millisLanzador(){
	...
	if(init_cont_barrera == 1){cont_barrera++;}
...
void ceroLanzador(void (*callback)(void)){
	...
	if (cont_cero < 2000){
		...
		if(flancos_detectados & (1 << SW4) || (cont_barrera > 500)){

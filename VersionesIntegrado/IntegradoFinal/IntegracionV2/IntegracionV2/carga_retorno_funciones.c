...
void botonPulsado(void(*callback)(void)){
	PCMSK0 &= ~(1 << PCINT7);
	habilita = 2;
	boton = 1;
	funcion_disparo = callback; // Guardamos para usarlo después
}
...
void setup_carga_retorno(void (*funcion_integracion)(void)){
	...
	PCMSK0 |= ((1 << PCINT0)|(1 << PCINT1)|(1 << PCINT7));	//Habilitar interrupciones especificas
	sei();
}

...
volatile uint8_t estado_anterior = 0xFF;	// cosas de Angel
volatile uint8_t estado_actual = 0;			// cosas de Angel

static void (*funcion_disparo)(void) = 0;
...
void setup_carga_retorno(void (*funcion_integracion)(void)){
	cli();
	
	funcion_disparo = funcion_integracion; //ASIGNA LA FUNCION A LLAMAR
...
ISR(TIMER3_OVF_vect){
	if(habilita==2){					//Comprueba si tiene permiso a rehabilitar las interrupciones, tras 50 ms
		if(boton){ PCMSK0 |= (1 << PCINT7); boton = 0; funcion_disparo();}
		else PCMSK0 |= ((1 << PCINT0) | (1 << PCINT1));	//Habilita las interrupciones especificas
		habilita = 0;					//Cancela la habilitación
	}
	if(habilita==1){TCNT3=0x3CAF; habilita=2;}				//Prepara el contador para los 50 ms
}
...
void finalCarga(){
	...
	if (motor_carga==2){		//Comprueba si estaba bajando		//Avisa de que el motor pasa a estar encendido subiendo

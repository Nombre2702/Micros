...
volatile uint8_t estado_anterior = 0x3F;  // Todos en reposo (1) -> 00111111
...
ISR(PCINT2_vect) {
	...
	for (uint8_t i = 0; i < NUM_BOLOS; i++) {
		...
		if ((estado_anterior & m) && !(estado_actual & m)) {
...
int main(void) {
	...
	while (1) {
		...
		uint8_t score = obtener_puntaje_total();
